...
/* 每页 35 个图标的切片模型，转发源模型信号以保留移动动画 */
class SliceModel : public QAbstractListModel {
    Q_OBJECT
public:
    static constexpr int PER_PAGE = 35;

    SliceModel(AppModel *src, int page);
...
private:
    void forwardMove(int from, int to) {
        const int start = m_page * PER_PAGE;
        const int lo = qMin(from, to), hi = qMax(from, to);
        if (lo >= start && hi < start + PER_PAGE) {
            // 同页移动：转发以触发 GridView 的 move 过渡动画
            const int lf = from - start, lt = to - start;
            beginMoveRows(QModelIndex(), lf, lf, QModelIndex(),
                          to > from ? lt + 1 : lt);
            endMoveRows();
        } else {
...
public:
    enum Roles { NameRole = Qt::UserRole + 1, IconRole, IdRole, RowRole };
    ...
    int rowCount(const QModelIndex & = {}) const override { return m_display.size(); }
    int count() const { return m_display.size(); }
    ...
    QVariant data(const QModelIndex &idx, int role) const override {
        ...
        if (row < 0 || row >= m_display.size()) return {};
        const AppInfo &a = m_apps[m_display[row]];
        switch (role) {
        case NameRole: return a.name;
        case IconRole: return a.icon;
        case IdRole:   return a.desktopId;
    ...
    QHash<int, QByteArray> roleNames() const override {
        return {{NameRole, "appName"}, {IconRole, "appIcon"}, {IdRole, "appId"}, {RowRole, "row"}};
    }

    Q_INVOKABLE QString idAt(int row) const {
        if (row < 0 || row >= m_display.size()) return QString();
        return m_apps[m_display[row]].desktopId;
    }

    Q_INVOKABLE QObject *pageModel(int page) {
        while (m_slices.size() <= page)
            m_slices << new SliceModel(this, m_slices.size());
        return m_slices[page];
    }

    Q_INVOKABLE void launch(const QString &desktopId) const {
        for (const AppInfo &a : m_apps) {
            if (a.desktopId != desktopId) continue;
            QString exec = a.exec;
    ...
    // 拖拽排序：from/to 为当前显示行号（仅未过滤时允许）
    Q_INVOKABLE void move(int from, int to) {
        if (!m_filter.isEmpty()) return;
        if (from < 0 || to < 0 || from >= m_display.size() || to >= m_display.size()) return;
        if (from == to) return;
        const int appFrom = m_display[from];
        // 用 id 映射做移动
        beginMoveRows(QModelIndex(), from, from, QModelIndex(), to > from ? to + 1 : to);
        m_display.move(from, to);
        // 重建 m_apps 顺序（display 未过滤时就是 0..n-1 的排列）
        QVector<AppInfo> newApps;
        newApps.reserve(m_apps.size());
        for (int i = 0; i < m_display.size(); ++i) newApps << m_apps[m_display[i]];
        // appFrom 已不再需要
        m_apps = newApps;
        for (int i = 0; i < m_apps.size(); ++i) m_display[i] = i;
        endMoveRows();
        Q_UNUSED(appFrom);
    }

    Q_INVOKABLE void saveOrder() {
        QSettings s(configDir() + "/settings.conf", QSettings::IniFormat);
        QStringList ids;
        ids.reserve(m_apps.size());
        for (const AppInfo &a : m_apps) ids << a.desktopId;
        s.setValue("order", ids.join(','));
        s.sync();
    }
...
private:
    ...
    void loadOrder() {
        ...
        for (int i = 0; i < order.size(); ++i) pos.insert(order[i], i);
        std::stable_sort(m_apps.begin(), m_apps.end(), [&pos](const AppInfo &a, const AppInfo &b) {
            const int pa = pos.value(a.desktopId, INT_MAX), pb = pos.value(b.desktopId, INT_MAX);
            if (pa == pb && pa == INT_MAX)
                return a.name.localeAwareCompare(b.name) < 0;
            return pa < pb;
        });
    ...
    void rebuild() {
        ...
        for (int i = 0; i < m_apps.size(); ++i) {
            if (m_filter.isEmpty() ||
                m_apps[i].name.contains(m_filter, Qt::CaseInsensitive) ||
                m_apps[i].desktopId.contains(m_filter, Qt::CaseInsensitive))
                m_display << i;
        }
    }

    QVector<AppInfo> m_apps;
    QVector<int> m_display;
    QString m_filter;
    bool m_loaded = false;
    QVector<SliceModel *> m_slices;
...
/* SliceModel 方法实现（需 AppModel 完整定义） */
SliceModel::SliceModel(AppModel *src, int page)
    : QAbstractListModel(src), m_src(src), m_page(page) {
    auto hard = [this]() { beginResetModel(); endResetModel(); };
    connect(src, &QAbstractListModel::modelReset, this, hard);
    connect(src, &QAbstractListModel::rowsInserted, this, hard);
    connect(src, &QAbstractListModel::rowsRemoved, this, hard);
    connect(src, &QAbstractListModel::rowsMoved, this,
            [this](const QModelIndex &, int srcFrom, int srcTo,
                   const QModelIndex &, int) { forwardMove(srcFrom, srcTo); });
}

int SliceModel::rowCount(const QModelIndex &) const {
    const int total = m_src->rowCount();
    const int start = m_page * PER_PAGE;
    return total > start ? qMin(PER_PAGE, total - start) : 0;
}

QVariant SliceModel::data(const QModelIndex &idx, int role) const {
    const int start = m_page * PER_PAGE;
    return m_src->data(m_src->index(start + idx.row()), role);
}

QHash<int, QByteArray> SliceModel::roleNames() const { return m_src->roleNames(); }
